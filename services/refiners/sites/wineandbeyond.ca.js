// services/refiners/sites/wineandbeyond.ca.js
...
export default async function refine(rootUrl, product, page) {
  ...
  const scraped = await page.evaluate(() => {
        ...
        try {
          ...
          for (const obj of list) {
            ...
            const countryCandidate =
              productNode.countryOfOrigin ||
              productNode.country ||
              productNode?.productionPlace?.name ||
              productNode?.origin?.name ||
              productNode?.manufacturer?.address?.addressCountry ||
              productNode?.brand?.address?.addressCountry;

            if (typeof countryCandidate === "string") return norm(countryCandidate);
            if (countryCandidate && typeof countryCandidate.name === "string") return norm(countryCandidate.name);
          }
        } catch {
          // ignore malformed scripts
        }
    ...
    const country = getByLabel("country of origin", "origin", "made in", "country") || jsonLdCountry;
    ...
    return { energy, sugar, abv, producer, country, category, vegan, gluten };
  });
