// services/refiners/sites/keinundlow.at.js
export default async function refine(rootUrl, product, page) {
    ...
    const extras = await page.evaluate(() => {
        ...
        const details = Array.from(document.querySelectorAll('collapsible-row details, details'));

        const getValue = (targetMatches) => {
            for (const detail of details) {
                const summaryText = normalize(detail.querySelector('summary')?.textContent);
                if (targetMatches.some(t => summaryText.includes(t))) {
                    const content = detail.querySelector('.collapsible__content, .accordion__content, .rte');
                    return content?.textContent?.replace(/\s+/g, ' ').trim() || null;
                }
            }
            return null;
        };
        ...
        return {
            ingredients: getValue(['zutaten', 'ingredients']),
            allergens: getValue(['allergene', 'allergens'])
        };
    });
